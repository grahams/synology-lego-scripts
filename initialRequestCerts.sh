...
docker run --rm -e LINODE_TOKEN_FILE=/ssl/linode-key -e LINODE_POLLING_INTERVAL=60 -e LINODE_PROPAGATION_TIMEOUT=0 -v ${DIR}:/ssl goacme/lego -a -k rsa4096 -d ${LE_DOMAIN} -m ${LE_EMAIL_ADDRESS} --dns=linodev4 --path=/ssl/.lego run
